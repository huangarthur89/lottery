    ...
    with col_ai:
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                user_nums_str = st.text_input("💡 (可選) 加入您的自選心水號碼 (請用空格或逗號分隔)：", placeholder="例如: 01 05 12 24 38")
                user_nums_input = []
                if user_nums_str:
                    import re
                    nums = re.findall(r'\d+', user_nums_str)
                    user_nums_input = sorted(list(set([int(x) for x in nums if 1 <= int(x) <= 49])))
                if user_nums_input:
                    st.caption(f"✅ 系統已成功讀取 **{len(user_nums_input)}** 顆號碼： {', '.join([str(x).zfill(2) for x in user_nums_input])}")
        ...
        # 讓使用者輸入自選號碼
        with st.container(border=True):
            user_nums_bazi_str = st.text_input("💡 請輸入 1~10 個您的心水號碼 (請用空格或逗號分隔)：", placeholder="例如: 01 05 12 34")
            user_nums_bazi = []
            if user_nums_bazi_str:
                import re
                nums = re.findall(r'\d+', user_nums_bazi_str)
                user_nums_bazi = sorted(list(set([int(x) for x in nums if 1 <= int(x) <= 49])))
            if user_nums_bazi:
                st.caption(f"✅ 系統已成功讀取 **{len(user_nums_bazi)}** 顆號碼： {', '.join([str(x).zfill(2) for x in user_nums_bazi])}")
