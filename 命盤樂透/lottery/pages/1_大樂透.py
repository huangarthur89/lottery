...
if not full_df.empty:
    ...
    with col_ai:
        ...
        if WHEELS:
            with st.form(key="wheel_form_lotto", clear_on_submit=False):
                wheel_options = list(WHEELS.keys())
                selected_wheel = st.selectbox("請選擇您的包牌策略與預算：", wheel_options, key="wheel_box_lotto")
                
                st.markdown("💡 **(可選) 加入您的自選心水號碼 (可多選)：**")
                user_nums_input = st.pills(
                    "請點選心水號碼", 
                    options=list(range(1, 50)), 
                    format_func=lambda x: str(x).zfill(2), 
                    selection_mode="multi", 
                    key="wheel_lotto_pills", 
                    label_visibility="collapsed"
                )
                
                if not user_nums_input:
                    user_nums_input = []
                    
                pure_manual = st.checkbox("🚫 純手工包牌模式 (完全不混入 AI 號碼，需點選足額號碼)", key="pure_manual_lotto")
