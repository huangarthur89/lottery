    ...
    with col_ai:
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                def render_wheel_lotto():
                    selected_pills = st.session_state.get("wheel_lotto_pills", [])
                    st.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(目前已選：{len(selected_pills)} 個)*")
                    with st.expander("📝 點此展開「選號區」", expanded=False):
                        st.pills("請選擇", options=list(range(1, 50)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="wheel_lotto_pills", label_visibility="collapsed")
                
                render_wheel_lotto()
                user_nums_input = st.session_state.get("wheel_lotto_pills", [])
        ...
        with st.container(border=True):
            ...
            def render_bazi_lotto():
                selected_bazi_pills = st.session_state.get("bazi_lotto_pills", [])
                st.markdown(f"**請選擇 1~10 個您的心水號碼：** *(目前已選：{len(selected_bazi_pills)} 個)*")
                with st.expander("📝 點此展開「選號區」", expanded=False):
                    st.pills("請選擇", options=list(range(1, 50)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="bazi_lotto_pills", label_visibility="collapsed")
            
            render_bazi_lotto()
            user_nums_bazi = st.session_state.get("bazi_lotto_pills", [])
