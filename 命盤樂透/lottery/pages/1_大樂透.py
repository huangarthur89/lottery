    ...
    with col_ai:
            ...
            with st.expander("📝 點此展開「彩券畫記區」", expanded=False):
                ...
                for i in range(1, 50):
                    with bazi_cols[(i-1) % 7]:
                        if st.checkbox(str(i).zfill(2), value=(i in default_bazi), key=f"bazi_lotto_{i}"):
        ...
        if st.button("🔮 解析夢境並產生推薦", key="top_dream_btn"):
            ...
            if not dream_text:
                ...
                dream_nums = list(set([n for n in extracted_numbers if 1 <= n <= 49]))
