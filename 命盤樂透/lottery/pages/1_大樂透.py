...
# 組合列資料 (只保留預計)
        ...
        # 找出出現頻率最高的前 12 顆一般號與前 2 顆特別號 (AI 投票機制與大數據互補)
        counts_n = Counter(all_n).most_common()
        master_n_1 = sorted([x[0] for x in counts_n[:6]])
        
        master_n_2_raw = [x[0] for x in counts_n[6:12]]
        if len(master_n_2_raw) < 6:
            # 若策略票選出的號碼不夠，以歷史全大數據熱門號碼補足
            all_nums_flat = full_df[['N1', 'N2', 'N3', 'N4', 'N5', 'N6']].values.flatten()
        ...
        counts_sp = Counter(all_sp).most_common()
        master_sp_1 = counts_sp[0][0]
        
        master_sp_2_raw = [x[0] for x in counts_sp[1:2]]
        if len(master_sp_2_raw) < 1:
            hist_sp_freq = [x[0] for x in Counter(full_df['特別號']).most_common()]
            for hsp in hist_sp_freq:
                if hsp != master_sp_1:
                    master_sp_2_raw.append(hsp)
                    break
        master_sp_2 = master_sp_2_raw[0] if master_sp_2_raw else random.randint(1, 49)
        
        st.markdown("### 🏆 綜合策略【下期預測】終極精選 (雙主推)")
        st.info("💡 系統已自動將上方 9 大 AI 策略所產生的號碼進行『交叉比對與投票計算』，並結合大數據推算出最強大的兩組推薦號碼：")
        
        def fmt_master_b(n): return f"<span style='display:inline-block; width:38px; height:38px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#262730; color:white; border-radius:50%; margin:5px; box-shadow: 2px 2px 5px rgba(0,0,0,0.3);'>{str(n).zfill(2)}</span>"
        def fmt_master_sp(n): return f"<span style='display:inline-block; width:38px; height:38px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#FF4B4B; color:white; border-radius:50%; margin:5px; box-shadow: 2px 2px 5px rgba(0,0,0,0.3);'>{str(n).zfill(2)}</span>"
        
        html_1 = "".join([fmt_master_b(n) for n in master_n_1]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_master_sp(master_sp_1)
        html_2 = "".join([fmt_master_b(n) for n in master_n_2]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_master_sp(master_sp_2)
        
        st.markdown(f"""
        <div style='display:flex; justify-content: space-around; flex-wrap: wrap;'>
            <div style='text-align: center; padding: 20px; background-color: #F0F2F6; border-radius: 10px; margin-top: 10px; flex: 1; min-width: 300px; margin-right: 10px;'>
                <h4 style='margin-bottom: 15px; color: #1A237E;'>🥇 第一組：AI 票選極熱共識</h4>
                {html_1}
            </div>
            <div style='text-align: center; padding: 20px; background-color: #F8FAFC; border-radius: 10px; margin-top: 10px; flex: 1; min-width: 300px; border: 1px solid #E0E0E0;'>
                <h4 style='margin-bottom: 15px; color: #4A148C;'>🥈 第二組：大數據潛力互補</h4>
                {html_2}
            </div>
        </div>
        """, unsafe_allow_html=True)
