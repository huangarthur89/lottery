...
if not full_df.empty:
    ...
    with col_ai:
        ...
        # 取得動態權重
        strategy_weights = calculate_strategy_weights(full_df, pool_size=49)
        
        # 產生本期主推
        curr_m_n_1, curr_m_n_2, curr_m_sp_1, curr_m_sp_2 = generate_master_prediction(curr_preds, strategy_weights, pool_size=49, df=full_df, special_pool_size=49)
        ...
        if len(full_df) > 1:
            ...
            prev_strategy_weights = calculate_strategy_weights(prev_df, pool_size=49)
            prev_m_n_1, prev_m_n_2, prev_m_sp_1, prev_m_sp_2 = generate_master_prediction(prev_preds, prev_strategy_weights, pool_size=49, df=prev_df, special_pool_size=49)
        else:
...
# 組合列資料 (只保留預計)
        ...
        def calc_prize(hit_n, hit_sp):
            if hit_n == 6: return 100000000
            elif hit_n == 5 and hit_sp: return 2000000
            elif hit_n == 5: return 60000
            elif hit_n == 4 and hit_sp: return 15000
            elif hit_n == 4: return 2000
            elif hit_n == 3 and hit_sp: return 1000
            elif hit_n == 2 and hit_sp: return 400
            elif hit_n == 3: return 400
            return 0
        ...
        with col_p1:
            ...
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px; margin-bottom:10px;">
                ...
                <div style="text-align:center; font-size:14px; font-weight:bold; color:#0068C9;">🎯 命中 {p_hit_1} 球 + {'1 特別號' if p_sp_hit_1 else '0 特別號'} | 準確率：{acc_1_pct}% | 💰 預計獎金：${prize_1:,}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_p2:
            ...
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px; margin-bottom:10px;">
                ...
                <div style="text-align:center; font-size:14px; font-weight:bold; color:#0068C9;">🎯 命中 {p_hit_2} 球 + {'1 特別號' if p_sp_hit_2 else '0 特別號'} | 準確率：{acc_2_pct}% | 💰 預計獎金：${prize_2:,}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # 讓使用者輸入自選號碼
        user_input_str = st.text_input("請選擇 1~10 個您的心水號碼 (用逗號隔開)：", "2, 4, 7, 9, 11, 30, 35, 42")
        ...
        if st.button("🚀 開始 AI 智慧微調分析"):
            ...
            try:
                ...
                freq_map = {n: random.randint(1, 100) for n in range(1, 50)}
            ...
            if use_bazi and bazi_lucky_pool:
                # 大樂透特別號優先從五行池挑選
                special_num = random.choice([n for n in bazi_lucky_pool]) 
            else:
                special_num = random.choice([n for n, c in Counter(df_hist['特別號']).most_common(5)]) if 'df_hist' in locals() and not df_hist.empty else random.randint(1, 49)
        ...
        if st.button("🔮 解析夢境並產生推薦", key="top_dream_btn"):
            ...
            if not dream_text:
                ...
                valid_base_nums = list(set([n for n in extracted_numbers if 1 <= n <= 49]))
                final_raw = expand_dream_resonance(dream_text, valid_base_nums, 49)
                dream_nums = [n for n in final_raw if 1 <= n <= 49]
                ...
                while len(final_base) < 6:
                    if candidate_pool:
                        ...
                        r_n = random.randint(1, 49) # 威力彩記得改 38
                        if r_n not in final_base:
                            final_base.append(r_n)
                            ai_added_nums.append(r_n)
                ...
                # 7. 特別號生成邏輯
                if use_bazi_dream and bazi_lucky_pool_dream:
                    special_num = random.choice([n for n in bazi_lucky_pool_dream])
                else:
                    special_num = random.choice([n for n, c in Counter(df_hist['特別號']).most_common(5)]) if 'df_hist' in locals() and not df_hist.empty else random.randint(1, 49)
