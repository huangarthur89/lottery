        ...
        if st.button("🔮 解析夢境並產生推薦", key="top_dream_btn"):
            ...
            if not dream_text:
                ...
                try:
                    ...
                    # 🔒 鎖定隨機數種子
                    last_draw_date = pd.to_datetime(df_hist.iloc[0]['日期'])
                    random.seed(int(time.mktime(last_draw_date.timetuple())) + sum([ord(c) for c in user_dream]))
                except:
                    random.seed(datetime.date.today().toordinal() + sum([ord(c) for c in user_dream]))
                    freq_map = {n: random.randint(1, 100) for n in range(1, 39)}
