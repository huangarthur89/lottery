...
if not full_df.empty:
    ...
    with col_ai:
        ...
        if WHEELS:
            with st.form(key="wheel_form_super", clear_on_submit=False):
                wheel_options = list(WHEELS.keys())
                selected_wheel = st.selectbox("請選擇您的包牌策略與預算：", wheel_options, key="wheel_box_super")
                
                st.markdown("### 威力彩選號區")
                
                st.markdown("💡 **(可選) 第一區心水號碼 (可多選)：**")
                user_nums_input = st.pills(
                    "第一區", 
                    options=list(range(1, 39)), 
                    format_func=lambda x: str(x).zfill(2), 
                    selection_mode="multi", 
                    key="wheel_super_pills_1", 
                    label_visibility="collapsed"
                )
                
                st.markdown("💡 **(可選) 第二區專屬特別號 (最多選 1 顆)：**")
                user_nums_b_input = st.pills(
                    "第二區", 
                    options=list(range(1, 9)), 
                    format_func=lambda x: str(x).zfill(2), 
                    selection_mode="multi", 
                    key="wheel_super_pills_2", 
                    label_visibility="collapsed"
                )
                
                if not user_nums_input: user_nums_input = []
                if not user_nums_b_input: user_nums_b_input = []
