...
def strategy_machine_learning(df, pool_size=38):
    ...
    try:
        ...
        from collections import Counter
        all_nums = df[['N1', 'N2', 'N3', 'N4', 'N5', 'N6']].values.flatten()
        return sorted([x[0] for x in Counter(all_nums).most_common(6)])
        
    train_size = min(len(df) - 1, 200)
    if train_size < 10:
        from collections import Counter
        all_nums = df[['N1', 'N2', 'N3', 'N4', 'N5', 'N6']].values.flatten()
        return sorted([x[0] for x in Counter(all_nums).most_common(6)])
    ...
    history_arr = np.zeros((len(df), pool_size + 1), dtype=bool)
    for i in range(len(df)):
        row_nums = df.iloc[i][num_cols].values
        for n in row_nums:
            history_arr[i, int(n)] = True

    for i in range(1, train_size):
        for num in range(1, pool_size + 1):
            target = 1 if history_arr[i, num] else 0
            lag_1 = 1 if (i+1 < len(df) and history_arr[i+1, num]) else 0
            lag_2 = 1 if (i+2 < len(df) and history_arr[i+2, num]) else 0
            end_idx = min(i + 11, len(df))
            freq_10 = np.sum(history_arr[i+1:end_idx, num])
            past_appearances = np.where(history_arr[i+1:, num])[0]
            omission = past_appearances[0] if len(past_appearances) > 0 else 100
            X_train.append([lag_1, lag_2, freq_10, omission])
    ...
    X_test = []
    for num in range(1, pool_size + 1):
        lag_1 = 1 if (0 < len(df) and history_arr[0, num]) else 0
        lag_2 = 1 if (1 < len(df) and history_arr[1, num]) else 0
        end_idx = min(10, len(df))
        freq_10 = np.sum(history_arr[0:end_idx, num])
        past_appearances = np.where(history_arr[0:, num])[0]
        omission = past_appearances[0] if len(past_appearances) > 0 else 100
        X_test.append([lag_1, lag_2, freq_10, omission])
        
    probs = rf.predict_proba(X_test)
    if probs.shape[1] > 1:
        probs = probs[:, 1]
    else:
        probs = probs[:, 0] # Failsafe
    ...
    with col_ai:
        ...
        def get_meta_prediction(preds_list):
            ...
            while len(top_6) < 6:
                ...
                top_6.append(random.choice(list(pool)))
        ...
        base_preds = [
            ...
            [*ml_6, random.choice(special_pool)]
        ]
        ...
        def generate_all_prev_preds():
            if len(full_df) > 1:
                ...
                base_prev_preds = [
                    ...
                    [*prev_ml_6, random.choice(prev_special_pool)]
                ]
        ...
        def get_top_n_from_preds(preds_list, n):
            ...
            while len(top_n) < n:
                ...
                top_n.append(random.choice(list(pool)))
            return sorted(top_n[:n])
            
        if WHEELS:
            with st.form("wheel_form_super"):
                wheel_options = list(WHEELS.keys())
                selected_wheel = st.selectbox("請選擇您的包牌策略與預算：", wheel_options, key="wheel_box_super")
                
                st.markdown("💡 **(可選) 加入您的自選心水號碼：**")
                user_nums_input = []
                with st.expander("📝 點此展開「彩券畫記區 (第一區)」", expanded=False):
                    grid_cols = st.columns(8)
                    for i in range(1, 39):
                        with grid_cols[(i-1) % 8]:
                            if st.checkbox(str(i).zfill(2), key=f"wheel_super_{i}"):
                                user_nums_input.append(i)
                                
                user_nums_b_input = []
                with st.expander("📝 點此展開「彩券畫記區 (第二區)」", expanded=False):
                    st.caption("請選擇 1 顆您的專屬特別號 (若不選將由 AI 隨機挑選)")
                    grid_cols_b = st.columns(8)
                    for i in range(1, 9):
                        with grid_cols_b[(i-1) % 8]:
                            if st.checkbox(str(i).zfill(2), key=f"wheel_super_b_{i}"):
                                user_nums_b_input.append(i)
                                
                pure_manual = st.checkbox("🚫 純手工包牌模式 (完全不混入 AI 號碼，需輸入足額號碼)", key="pure_manual_super")
                
                submitted = st.form_submit_button("🚀 立即產生威力彩聰明注單")
            ...
            if submitted and selected_wheel:
                ...
                if pure_manual:
                    if len(user_nums) < req_n:
                        ...
                        import random
                        chosen_b = user_nums_b_input[0] if user_nums_b_input else random.randint(1, 8)
                        cols = st.columns(4)
                    ...
                    if user_nums:
                        ...
                        while len(combined) < req_n:
                            ...
                            combined.append(random.choice(list(pool)))
        ...
        # 讓使用者輸入自選號碼
        with st.form("bazi_form_super"):
            st.markdown("**請選擇 1~10 個您的心水號碼：**")
            user_nums_bazi = []
            with st.expander("📝 點此展開「彩券畫記區 (第一區)」", expanded=False):
                bazi_cols = st.columns(8)
                default_bazi = [2, 4, 7, 9, 11, 30]
                for i in range(1, 39):
                    with bazi_cols[(i-1) % 8]:
                        if st.checkbox(str(i).zfill(2), value=(i in default_bazi), key=f"bazi_super_{i}"):
                            user_nums_bazi.append(i)
                            
            user_nums_bazi_b = []
            with st.expander("📝 點此展開「彩券畫記區 (第二區)」", expanded=False):
                st.caption("請選擇 1 顆您的專屬特別號 (若不選將由 AI 隨機挑選)")
                bazi_cols_b = st.columns(8)
                for i in range(1, 9):
                    with bazi_cols_b[(i-1) % 8]:
                        if st.checkbox(str(i).zfill(2), key=f"bazi_super_b_{i}"):
                            user_nums_bazi_b.append(i)
            ...
            submit_bazi = st.form_submit_button("🚀 開始 AI 智慧微調分析")
