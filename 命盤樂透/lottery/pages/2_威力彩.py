    ...
    with col_ai:
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                def render_wheel_super():
                    wheel_super_count = sum(1 for i in range(1, 39) if st.session_state.get(f"wheel_super_{i}", False))
                    wheel_super_b_count = sum(1 for i in range(1, 9) if st.session_state.get(f"wheel_super_b_{i}", False))
                    st.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(第一區已選：{wheel_super_count} 個，第二區已選：{wheel_super_b_count} 個)*")
                    with st.expander("📝 點此展開「彩券畫記區 (第一區)」", expanded=False):
                        grid_cols = st.columns(8)
                        for i in range(1, 39):
                            with grid_cols[(i-1) % 8]:
                                st.checkbox(str(i).zfill(2), key=f"wheel_super_{i}")
                    ...
                    with st.expander("📝 點此展開「彩券畫記區 (第二區)」", expanded=False):
                        ...
                        for i in range(1, 9):
                            with grid_cols_b[(i-1) % 8]:
                                st.checkbox(str(i).zfill(2), key=f"wheel_super_b_{i}")
                
                render_wheel_super()
                user_nums_input = [i for i in range(1, 39) if st.session_state.get(f"wheel_super_{i}", False)]
                user_nums_b_input = [i for i in range(1, 9) if st.session_state.get(f"wheel_super_b_{i}", False)]
        ...
        with st.container(border=True):
            ...
            def render_bazi_super():
                bazi_super_count = sum(1 for i in range(1, 39) if st.session_state.get(f"bazi_super_{i}", False))
                bazi_super_b_count = sum(1 for i in range(1, 9) if st.session_state.get(f"bazi_super_b_{i}", False))
                st.markdown(f"**請選擇 1~10 個您的心水號碼：** *(第一區已選：{bazi_super_count} 個，第二區已選：{bazi_super_b_count} 個)*")
                with st.expander("📝 點此展開「彩券畫記區 (第一區)」", expanded=False):
                    bazi_cols = st.columns(8)
                    default_bazi = []
                    for i in range(1, 39):
                        with bazi_cols[(i-1) % 8]:
                            st.checkbox(str(i).zfill(2), value=(i in default_bazi), key=f"bazi_super_{i}")
                ...
                with st.expander("📝 點此展開「彩券畫記區 (第二區)」", expanded=False):
                    ...
                    for i in range(1, 9):
                        with bazi_cols_b[(i-1) % 8]:
                            st.checkbox(str(i).zfill(2), key=f"bazi_super_b_{i}")

            render_bazi_super()
            user_nums_bazi = [i for i in range(1, 39) if st.session_state.get(f"bazi_super_{i}", False)]
            user_nums_bazi_b = [i for i in range(1, 9) if st.session_state.get(f"bazi_super_b_{i}", False)]
