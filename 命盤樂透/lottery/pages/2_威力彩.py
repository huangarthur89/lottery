    ...
    with col_ai:
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                with col1:
                    user_nums_str = st.text_input("💡 自選心水號碼 (第一區 1~38)：", placeholder="空格分隔, 如: 01 05 12")
                with col2:
                    user_nums_b_str = st.text_input("💡 專屬特別號 (第二區 1~8)：", placeholder="最多 1 顆, 如: 05")
                
                user_nums_input = []
                user_nums_b_input = []
                import re
                if user_nums_str:
                    nums = re.findall(r'\d+', user_nums_str)
                    user_nums_input = sorted(list(set([int(x) for x in nums if 1 <= int(x) <= 38])))
                if user_nums_b_str:
                    nums_b = re.findall(r'\d+', user_nums_b_str)
                    user_nums_b_input = sorted(list(set([int(x) for x in nums_b if 1 <= int(x) <= 8])))
                
                if user_nums_input or user_nums_b_input:
                    msg = "✅ 系統已成功讀取："
                    if user_nums_input:
                        msg += f" [第一區 {len(user_nums_input)} 顆] {', '.join([str(x).zfill(2) for x in user_nums_input])}"
                    if user_nums_b_input:
                        msg += f" [第二區 {len(user_nums_b_input)} 顆] {', '.join([str(x).zfill(2) for x in user_nums_b_input])}"
                    st.caption(msg)
        ...
        with st.container(border=True):
            col1, col2 = st.columns(2)
            with col1:
                bazi_nums_str = st.text_input("💡 請輸入您的心水號碼 (第一區 1~38)：", placeholder="空格分隔, 如: 01 05 12")
            with col2:
                bazi_nums_b_str = st.text_input("💡 專屬特別號 (第二區 1~8)：", placeholder="最多 1 顆, 如: 05")
            
            user_nums_bazi = []
            user_nums_bazi_b = []
            import re
            if bazi_nums_str:
                nums = re.findall(r'\d+', bazi_nums_str)
                user_nums_bazi = sorted(list(set([int(x) for x in nums if 1 <= int(x) <= 38])))
            if bazi_nums_b_str:
                nums_b = re.findall(r'\d+', bazi_nums_b_str)
                user_nums_bazi_b = sorted(list(set([int(x) for x in nums_b if 1 <= int(x) <= 8])))
            
            if user_nums_bazi or user_nums_bazi_b:
                msg = "✅ 系統已成功讀取："
                if user_nums_bazi:
                    msg += f" [第一區 {len(user_nums_bazi)} 顆] {', '.join([str(x).zfill(2) for x in user_nums_bazi])}"
                if user_nums_bazi_b:
                    msg += f" [第二區 {len(user_nums_bazi_b)} 顆] {', '.join([str(x).zfill(2) for x in user_nums_bazi_b])}"
                st.caption(msg)
