...
def calculate_bazi_lucky_nums(birth_date, birth_hour_str, pool_size=38):
    ...
    hour_branch = "午" if "吉時" in birth_hour_str else birth_hour_str[0]
...
def calculate_strategy_weights(df, pool_size):
    """計算各策略在過去 5 期的動態權重"""
    weights = [1.0] * 9
    if len(df) < 10: return weights
    for i in range(5):
        target_draw = df.iloc[i]
        target_nums = set([target_draw['N1'], target_draw['N2'], target_draw['N3'], target_draw['N4'], target_draw['N5'], target_draw['N6']])
        hist_df = df.iloc[i+1:].reset_index(drop=True)
        if hist_df.empty: break
        
        freq = Counter(hist_df[['N1', 'N2', 'N3', 'N4', 'N5', 'N6']].values.flatten())
        s0 = [x[0] for x in freq.most_common(6)]
        s1 = strategy_max_omission(hist_df, pool_size=pool_size)
        s2 = strategy_trailing_numbers(hist_df, pool_size=pool_size)
        s3 = strategy_golden_ratio(pool_size=pool_size, min_sum=130 if pool_size==49 else 95, max_sum=170 if pool_size==49 else 139)
        s4 = strategy_pattern_combo(hist_df, pool_size=pool_size)
        s5 = strategy_short_term_momentum(hist_df, pool_size=pool_size, recent_draws=50)
        s6 = strategy_zone_elimination(hist_df, pool_size=pool_size)
        s7 = strategy_composite_ai(hist_df, pool_size=pool_size)
        s8 = strategy_absolute_cold(hist_df, pool_size=pool_size)
        
        preds = [s0, s1, s2, s3, s4, s5, s6, s7, s8]
        for idx, p in enumerate(preds):
            hits = len(set(p).intersection(target_nums))
            weights[idx] += hits * 0.5
    return weights

def generate_master_prediction(preds, weights, pool_size, df, special_pool_size):
    scores_n = {n: 0.0 for n in range(1, pool_size + 1)}
    scores_sp = {n: 0.0 for n in range(1, special_pool_size + 1)}
    ...
    sorted_n = sorted(scores_n.items(), key=lambda x: x[1], reverse=True)
    master_n_1 = sorted([x[0] for x in sorted_n[:6]])
    
    master_n_2_raw = [x[0] for x in sorted_n[6:12]]
    if len(master_n_2_raw) < 6:
        all_nums_flat = df[['N1', 'N2', 'N3', 'N4', 'N5', 'N6']].values.flatten()
        hist_freq = [x[0] for x in Counter(all_nums_flat).most_common()]
        for hn in hist_freq:
            if hn not in master_n_1 and hn not in master_n_2_raw:
                master_n_2_raw.append(hn)
            if len(master_n_2_raw) == 6: break
    master_n_2 = sorted(master_n_2_raw)
    
    sorted_sp = sorted(scores_sp.items(), key=lambda x: x[1], reverse=True)
    master_sp_1 = sorted_sp[0][0]
    
    master_sp_2_raw = [sorted_sp[1][0]]
    if len(master_sp_2_raw) < 1:
        hist_sp_freq = [x[0] for x in Counter(df['特別號']).most_common()]
        for hsp in hist_sp_freq:
            if hsp != master_sp_1:
                master_sp_2_raw.append(hsp)
                break
    master_sp_2 = master_sp_2_raw[0] if master_sp_2_raw else random.randint(1, special_pool_size)
    
    return master_n_1, master_n_2, master_sp_1, master_sp_2
...
def load_from_db():
    try:
        ...
        db_path = Path(__file__).resolve().parents[2] / 'lottery_data.db'
...
if not full_df.empty:
    ...
    with col_ai:
        ...
        # --- 設定隨機亂數種子，確保預測結果的一致性 ---
        last_draw_date = pd.to_datetime(full_df.iloc[0]['日期'])
        random.seed(int(time.mktime(last_draw_date.timetuple())))
        ...
        curr_preds = [
            ...
            [*momentum_6, random.choice(special_pool)],
            [*zone_6, random.choice(special_pool)],    
            [*composite_6, random.choice(special_pool)],
            [*cold_6, random.choice(special_pool)]      
        ]
        
        # 取得動態權重
        strategy_weights = calculate_strategy_weights(full_df, pool_size=38)
        
        # 產生本期主推
        curr_m_n_1, curr_m_n_2, curr_m_sp_1, curr_m_sp_2 = generate_master_prediction(curr_preds, strategy_weights, pool_size=38, df=full_df, special_pool_size=8)
        
        # --- 前期預測 (基於扣除最新一期的歷史) ---
        if len(full_df) > 1:
            prev_df = full_df.iloc[1:].reset_index(drop=True)
            prev_draw_date = pd.to_datetime(prev_df.iloc[0]['日期'])
            random.seed(int(time.mktime(prev_draw_date.timetuple())))
            ...
            prev_strategy_weights = calculate_strategy_weights(prev_df, pool_size=38)
            prev_m_n_1, prev_m_n_2, prev_m_sp_1, prev_m_sp_2 = generate_master_prediction(prev_preds, prev_strategy_weights, pool_size=38, df=prev_df, special_pool_size=8)
        else:
            prev_preds = curr_preds
            prev_df = full_df
            prev_strategy_weights = strategy_weights
            prev_m_n_1, prev_m_n_2, prev_m_sp_1, prev_m_sp_2 = curr_m_n_1, curr_m_n_2, curr_m_sp_1, curr_m_sp_2
        ...
        strategy_names = [
            f"🔥 史上最熱門 (權重: {strategy_weights[0]:.1f}x)",
            f"⏳ 絕地反彈 (權重: {strategy_weights[1]:.1f}x)",
            f"🎯 上期拖牌 (權重: {strategy_weights[2]:.1f}x)",
            f"⚖️ 完美統計學 (權重: {strategy_weights[3]:.1f}x)",
            f"👯 連號與連莊 (權重: {strategy_weights[4]:.1f}x)",
            f"📈 近期動能 (權重: {strategy_weights[5]:.1f}x)",  
            f"🕳️ 斷區殺牌 (權重: {strategy_weights[6]:.1f}x)",   
            f"🧠 綜合 AI (權重: {strategy_weights[7]:.1f}x)",    
            f"❄️ 絕對冷門 (權重: {strategy_weights[8]:.1f}x)"      
        ]
...
# 組合列資料 (只保留預計)
        ...
        for row in combined_data:
            ...
            for num in row[1:7]:
                if num in last_draw_nums_set:
                    balls.append(f"<span style='color:#FF4B4B; font-weight:bold;'>{str(num).zfill(2)}</span>")
                else:
                    balls.append(str(num).zfill(2))
            
            # 威力彩特別號對中特別標紅
            sp_num = row[7]
            if sp_num == last_draw_row['特別號']:
                sp_html = f"<span style='color:#FF4B4B; font-weight:bold;'>{str(sp_num).zfill(2)}</span>"
            else:
                sp_html = str(sp_num).zfill(2)
                
            md_table += f"| {strategy} | {' | '.join(balls)} | {sp_html} | {row[8]} |\n"
            
        st.markdown(md_table, unsafe_allow_html=True)

        # ==========================================
        # 🏆 綜合九大策略的【上期預測】終極號碼 (回顧)
        # ==========================================

        
        # 上期預測績效計算
        p_hit_1 = len(set(prev_m_n_1).intersection(draw_nums))
        p_sp_hit_1 = (prev_m_sp_1 == special_num)
        
        p_hit_2 = len(set(prev_m_n_2).intersection(draw_nums))
        p_sp_hit_2 = (prev_m_sp_2 == special_num)
        
        # 準確率字串
        acc_1_pct = round((p_hit_1 + (1 if p_sp_hit_1 else 0)) / 7 * 100)
        acc_2_pct = round((p_hit_2 + (1 if p_sp_hit_2 else 0)) / 7 * 100)

        def calc_prize(hit_n, hit_sp):
            if hit_n == 6 and hit_sp: return 200000000 # 頭獎
            elif hit_n == 6: return 5000000 # 貳獎
            elif hit_n == 5 and hit_sp: return 150000 # 參獎
            elif hit_n == 5: return 20000 # 肆獎
            elif hit_n == 4 and hit_sp: return 4000 # 伍獎
            elif hit_n == 4: return 800 # 陸獎
            elif hit_n == 3 and hit_sp: return 400 # 柒獎
            elif hit_n == 2 and hit_sp: return 200 # 捌獎
            elif hit_n == 3: return 100 # 玖獎
            elif hit_n == 1 and hit_sp: return 100 # 普獎
            return 0
            
        prize_1 = calc_prize(p_hit_1, p_sp_hit_1)
        prize_2 = calc_prize(p_hit_2, p_sp_hit_2)
        
        prize_str_1 = f"＄{prize_1:,}" if prize_1 > 0 else "＄0"
        if p_hit_1 == 6 and p_sp_hit_1: prize_str_1 = "頭獎 (浮動)"
        elif p_hit_1 == 6: prize_str_1 = "貳獎 (浮動)"
        
        prize_str_2 = f"＄{prize_2:,}" if prize_2 > 0 else "＄0"
        if p_hit_2 == 6 and p_sp_hit_2: prize_str_2 = "頭獎 (浮動)"
        elif p_hit_2 == 6: prize_str_2 = "貳獎 (浮動)"
        
        def fmt_master_b(n): 
            is_hit = n in draw_nums
            style = "border: 3px solid #FF4B4B; box-shadow: 0 0 10px #FF4B4B; color: #FF4B4B;" if is_hit else "border: 3px solid transparent; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); color: white;"
            return f"<span style='display:inline-block; width:44px; height:44px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#262730; border-radius:50%; margin:5px; box-sizing:border-box; transition: 0.3s; {style}'>{str(n).zfill(2)}</span>"
            
        def fmt_master_sp(n): 
            is_hit = (n == special_num)
            style = "border: 3px solid #FFD700; box-shadow: 0 0 10px #FFD700; color: #FFD700;" if is_hit else "border: 3px solid transparent; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); color: white;"
            return f"<span style='display:inline-block; width:44px; height:44px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#FF4B4B; border-radius:50%; margin:5px; box-sizing:border-box; transition: 0.3s; {style}'>{str(n).zfill(2)}</span>"

        html_prev_1 = "".join([fmt_master_b(n) for n in prev_m_n_1]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_master_sp(prev_m_sp_1)
        html_prev_2 = "".join([fmt_master_b(n) for n in prev_m_n_2]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_master_sp(prev_m_sp_2)

        st.markdown("---")
        st.markdown("### 📊 系統績效驗證：【上期預測】終極精選回顧")
        st.info("💡 下方是系統在「上一期開獎前」所固定的預測號碼。我們將其與最新開獎結果進行比對，真實追蹤 AI 的勝率：")
        
        col_p1, col_p2 = st.columns(2)
        with col_p1:
            st.markdown(f"""
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px; margin-bottom:10px;">
                <h4 style="margin-top:0; color:#1F2937; text-align:center;">🥇 上期第一組：AI 票選極熱共識</h4>
                <div style="text-align:center; margin: 15px 0;">{html_prev_1}</div>
                <div style="text-align:center; font-size:14px; font-weight:bold; color:#0068C9;">🎯 命中 {p_hit_1} 球 + {'1 特別號' if p_sp_hit_1 else '0 特別號'} | 準確率：{acc_1_pct}% | 💰 預計獎金：{prize_str_1}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_p2:
            st.markdown(f"""
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px; margin-bottom:10px;">
                <h4 style="margin-top:0; color:#1F2937; text-align:center;">🥈 上期第二組：大數據潛力互補</h4>
                <div style="text-align:center; margin: 15px 0;">{html_prev_2}</div>
                <div style="text-align:center; font-size:14px; font-weight:bold; color:#0068C9;">🎯 命中 {p_hit_2} 球 + {'1 特別號' if p_sp_hit_2 else '0 特別號'} | 準確率：{acc_2_pct}% | 💰 預計獎金：{prize_str_2}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # 這裡為了展示不用加上中獎發光，所以使用乾淨的球
        def fmt_curr_b(n): 
            return f"<span style='display:inline-block; width:44px; height:44px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#262730; border-radius:50%; margin:5px; box-sizing:border-box; color: white;'>{str(n).zfill(2)}</span>"
            
        def fmt_curr_sp(n): 
            return f"<span style='display:inline-block; width:44px; height:44px; line-height:38px; text-align:center; font-size:20px; font-weight:bold; background-color:#FF4B4B; border-radius:50%; margin:5px; box-sizing:border-box; color: white;'>{str(n).zfill(2)}</span>"

        html_curr_1 = "".join([fmt_curr_b(n) for n in curr_m_n_1]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_curr_sp(curr_m_sp_1)
        html_curr_2 = "".join([fmt_curr_b(n) for n in curr_m_n_2]) + " <span style='font-size:24px; font-weight:bold; vertical-align:middle; margin:0 10px;'>➕</span> " + fmt_curr_sp(curr_m_sp_2)

        st.markdown("---")
        st.markdown("### 🚀 綜合策略：【下期預測】終極精選 (雙主推)")
        st.info("💡 以下號碼已使用時間種子鎖定，確保預測不變。系統自動將上方 9 大 AI 策略所產生的號碼進行『權重投票計算』，推算出最強大的兩組推薦號碼：")
        
        col1, col2 = st.columns(2)
        with col1:
            st.markdown(f"""
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px;">
                <h4 style="margin-top:0; color:#1F2937; text-align:center;">🥇 第一組：AI 票選極熱共識</h4>
                <div style="text-align:center; margin-top:15px;">{html_curr_1}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(f"""
            <div style="background-color:#F0F2F6; padding:15px; border-radius:10px;">
                <h4 style="margin-top:0; color:#1F2937; text-align:center;">🥈 第二組：大數據潛力互補</h4>
                <div style="text-align:center; margin-top:15px;">{html_curr_2}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # 讓使用者輸入自選號碼
        user_input_str = st.text_input("請選擇 1~10 個您的心水號碼 (用逗號隔開)：", "2, 4, 7, 9, 11, 30, 35, 37")
        ...
        if use_bazi:
            ...
            with b_col2:
                b_hour = st.selectbox("出生時辰：", ["吉時 (不知時辰)", "子時 (23-01)", "丑時 (01-03)", "寅時 (03-05)", "卯時 (05-07)", "辰時 (07-09)", "巳時 (09-11)", "午時 (11-13)", "未時 (13-15)", "申時 (15-17)", "酉時 (17-19)", "戌時 (19-21)", "亥時 (21-23)"])
        ...
        if st.button("🚀 開始 AI 智慧微調分析"):
            ...
            if len(user_input_nums) > 10 or not all(1 <= x <= 38 for x in user_input_nums):
                st.error("⚠️ 請確保最多輸入 10 個號碼，且都在 1~38 之間喔！")
                st.stop()
            ...
            try:
                ...
                # 🔒 鎖定隨機數種子
                last_draw_date = pd.to_datetime(df_hist.iloc[0]['日期'])
                random.seed(int(time.mktime(last_draw_date.timetuple())) + sum(user_input_nums))
            except:
                random.seed(datetime.date.today().toordinal() + sum(user_input_nums))
            ...
            # 根據使用者輸入的數量決定保留多少個好的號碼，剩下的空間讓給 AI 補強勢號
            if len(sorted_user_nums) >= 6:
                keep_len = random.choice([4, 5]) # 最多只保留 4~5 個，至少讓 AI 補 1~2 個
            elif len(sorted_user_nums) == 5:
                keep_len = random.choice([3, 4])
            elif len(sorted_user_nums) <= 4:
                keep_len = max(1, len(sorted_user_nums) - random.choice([0, 1])) # 隨機剔除 0~1 個
        ...
        if st.session_state.get('super_custom_res'):
            ...
            def format_ball_c(n):
                return f"<span style='font-size: 20px; font-weight:bold; background-color:#F0F2F6; padding: 4px 10px; border-radius: 50%; margin-right: 5px; color:#1F2937;'>{str(n).zfill(2)}</span>"
            def format_ai_ball_c(n):
            ...
            balls_html = ""
            for n in res['final_base']:
                if n in res['ai_added_nums']:
                    balls_html += format_ai_ball_c(n)
                else:
                    balls_html += format_ball_c(n)
                    
            st.markdown(f"**最終優化組合**： {balls_html} ➕ 特別號： {format_sp_ball_c(res['special_num'])}", unsafe_allow_html=True)
            if res['ai_added_nums']:
                st.caption("🔴 <span style='color:#FF4B4B; font-weight:bold;'>紅色號碼</span> 為系統大數據與八字磁場微調加入之強勢號碼", unsafe_allow_html=True)
        ...
        if use_bazi_dream:
            ...
            with b_col2:
                b_hour_dream = st.selectbox("出生時辰：", ["吉時 (不知時辰)", "子時 (23-01)", "丑時 (01-03)", "寅時 (03-05)", "卯時 (05-07)", "辰時 (07-09)", "巳時 (09-11)", "午時 (11-13)", "未時 (13-15)", "申時 (15-17)", "酉時 (17-19)", "戌時 (19-21)", "亥時 (21-23)"], key="dream_hour")
        ...
        if st.button("🔮 解析夢境並產生推薦", key="top_dream_btn"):
            ...
            if not dream_text:
                ...
                valid_base_nums = list(set([n for n in extracted_numbers if 1 <= n <= 38]))
                final_raw = expand_dream_resonance(dream_text, valid_base_nums, 38)
                dream_nums = [n for n in final_raw if 1 <= n <= 38]
                ...
                try:
                    ...
                    # 🔒 鎖定隨機數種子
                    last_draw_date = pd.to_datetime(df_hist.iloc[0]['日期'])
                    random.seed(int(time.mktime(last_draw_date.timetuple())) + sum([ord(c) for c in dream_text]))
                except:
                    random.seed(datetime.date.today().toordinal() + sum([ord(c) for c in dream_text]))
                    freq_map = {n: random.randint(1, 100) for n in range(1, 39)}
        ...
        if st.button("🩺 開始 AI 健檢"):
            if full_df.empty:
                ...
                try:
                    ...
                    # 🔒 鎖定隨機數種子
                    last_draw_date = pd.to_datetime(full_df.iloc[0]['日期'])
                    random.seed(int(time.mktime(last_draw_date.timetuple())) + sum(check_nums))
