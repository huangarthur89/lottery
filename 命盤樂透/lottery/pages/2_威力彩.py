    ...
    with col_ai:
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                @st.fragment
                def render_wheel_super():
                    selected_pills_1 = st.session_state.get("wheel_super_pills_1", [])
                    selected_pills_2 = st.session_state.get("wheel_super_pills_2", [])
                    st.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(第一區已選：{len(selected_pills_1)} 個，第二區已選：{len(selected_pills_2)} 個)*")
        ...
        with st.container(border=True):
            @st.fragment
            def render_bazi_super():
                selected_bazi_pills_1 = st.session_state.get("bazi_super_pills_1", [])
                selected_bazi_pills_2 = st.session_state.get("bazi_super_pills_2", [])
                st.markdown(f"**請選擇 1~10 個您的心水號碼：** *(第一區已選：{len(selected_bazi_pills_1)} 個，第二區已選：{len(selected_bazi_pills_2)} 個)*")
                with st.expander("📝 點此展開「選號區 (第一區)」", expanded=False):
