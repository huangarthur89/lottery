...
def strategy_machine_learning(df, pool_size=38):
...
if not full_df.empty:
    ...
    with col_ai:
        ...
        # 10. 可解釋特徵加權模型
        ml_6 = strategy_machine_learning(full_df, pool_size=38)
        ...
        stats_file = os.path.join(os.path.dirname(__file__), 'super_lotto_stats.json')
        stats_data = {}
        ...
        def generate_all_prev_preds():
            if len(full_df) > 1:
                ...
                prev_ml_6 = strategy_machine_learning(prev_df, pool_size=38)
        ...
        # 處理重新整理請求
        regen_idx = st.session_state.get('regen_idx_super', -1)
        if 'regen_idx_super' in st.session_state:
            del st.session_state['regen_idx_super']
        ...
        for i in range(11):
            ...
            if prize == 200000000: prize_str = "頭獎 (浮動)"
            elif prize == 5000000: prize_str = "貳獎 (浮動)"
        ...
        with regen_col1:
            ...
            selected_regen = st.selectbox("選擇要重新產生的對象（重新產生不計入歷史中獎次數）：", regen_options, label_visibility="collapsed", key="regen_super_box")
        with regen_col2:
            if st.button("🚀 重新產生所選預測", use_container_width=True, key="regen_super_btn"):
                if selected_regen == "全部策略":
                    st.session_state['regen_idx_super'] = 999
                else:
                    st.session_state['regen_idx_super'] = strategy_names.index(selected_regen)
                st.rerun()
        ...
        if WHEELS:
            with st.container(border=True):
                ...
                selected_wheel = st.selectbox("請選擇您的包牌策略與預算：", wheel_options, key="wheel_box_super")
                
                counter_placeholder = st.empty()
                with st.expander("📝 點此展開「選號區 (第一區)」", expanded=False):
                    @st.fragment
                    def render_pills_1():
                        st.pills("請選擇", options=list(range(1, 39)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="wheel_super_pills_1", label_visibility="collapsed")
                        p1 = st.session_state.get("wheel_super_pills_1", [])
                        p2 = st.session_state.get("wheel_super_pills_2", [])
                        counter_placeholder.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(第一區已選：{len(p1)} 個，第二區已選：{len(p2)} 個)*")
                    render_pills_1()
                                    
                with st.expander("📝 點此展開「選號區 (第二區)」", expanded=False):
                    @st.fragment
                    def render_pills_2():
                        st.caption("請選擇 1 顆您的專屬特別號 (若不選將依歷史熱度挑選)")
                        st.pills("請選擇", options=list(range(1, 9)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="wheel_super_pills_2", label_visibility="collapsed")
                        p1 = st.session_state.get("wheel_super_pills_1", [])
                        p2 = st.session_state.get("wheel_super_pills_2", [])
                        counter_placeholder.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(第一區已選：{len(p1)} 個，第二區已選：{len(p2)} 個)*")
                    render_pills_2()
                ...
                # 初始化計數顯示
                p1_init = st.session_state.get("wheel_super_pills_1", [])
                p2_init = st.session_state.get("wheel_super_pills_2", [])
                counter_placeholder.markdown(f"💡 **(可選) 加入您的自選心水號碼：** *(第一區已選：{len(p1_init)} 個，第二區已選：{len(p2_init)} 個)*")
                user_nums_input = st.session_state.get("wheel_super_pills_1", [])
                user_nums_b_input = st.session_state.get("wheel_super_pills_2", [])
                                
                pure_manual = st.checkbox("🚫 純手工包牌模式 (完全不混入 AI 號碼，需輸入足額號碼)", key="pure_manual_super")
                
                submitted = st.button("🚀 立即產生威力彩聰明注單", key="wheel_btn_super")
            ...
            if submitted and selected_wheel:
                ...
                cost = len(matrix) * 100 # 威力彩一注 100 元
                ...
                if pure_manual:
                    if len(user_nums) < req_n:
                        ...
                        chosen_b = user_nums_b_input[0] if user_nums_b_input else _top_special_from_df(full_df, 8)
                        cols = st.columns(4)
                        for i, t in enumerate(tickets):
                            with cols[i % 4]:
                                st.markdown(f"**第 {str(i+1).zfill(2)} 注**： `{' '.join([str(x).zfill(2) for x in t])}` + **[{str(chosen_b).zfill(2)}]**")
                else:
                    ...
                    if user_nums:
                        ...
                        while len(combined) < req_n:
                            pool = set(range(1, 39)) - set(combined)
        ...
        # 讓使用者輸入自選號碼
        with st.container(border=True):
            bazi_counter_placeholder = st.empty()
            with st.expander("📝 點此展開「選號區 (第一區)」", expanded=False):
                @st.fragment
                def render_bazi_pills_1():
                    st.pills("請選擇", options=list(range(1, 39)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="bazi_super_pills_1", label_visibility="collapsed")
                    bp1 = st.session_state.get("bazi_super_pills_1", [])
                    bp2 = st.session_state.get("bazi_super_pills_2", [])
                    bazi_counter_placeholder.markdown(f"**請選擇 1~10 個您的心水號碼：** *(第一區已選：{len(bp1)} 個，第二區已選：{len(bp2)} 個)*")
                render_bazi_pills_1()
                            
            with st.expander("📝 點此展開「選號區 (第二區)」", expanded=False):
                @st.fragment
                def render_bazi_pills_2():
                    st.caption("請選擇 1 顆您的專屬特別號 (若不選將依歷史熱度挑選)")
                    st.pills("請選擇", options=list(range(1, 9)), format_func=lambda x: str(x).zfill(2), selection_mode="multi", key="bazi_super_pills_2", label_visibility="collapsed")
                    bp1 = st.session_state.get("bazi_super_pills_1", [])
                    bp2 = st.session_state.get("bazi_super_pills_2", [])
                    bazi_counter_placeholder.markdown(f"**請選擇 1~10 個您的心水號碼：** *(第一區已選：{len(bp1)} 個，第二區已選：{len(bp2)} 個)*")
                render_bazi_pills_2()
            ...
            # 初始化計數顯示
            bp1_init = st.session_state.get("bazi_super_pills_1", [])
            bp2_init = st.session_state.get("bazi_super_pills_2", [])
            bazi_counter_placeholder.markdown(f"**請選擇 1~10 個您的心水號碼：** *(第一區已選：{len(bp1_init)} 個，第二區已選：{len(bp2_init)} 個)*")
            user_nums_bazi = st.session_state.get("bazi_super_pills_1", [])
            user_nums_bazi_b = st.session_state.get("bazi_super_pills_2", [])
            ...
            submit_bazi = st.button("🚀 開始 AI 智慧微調分析", key="bazi_btn_super")
        ...
        if submit_bazi:
            ...
            try:
                ...
                all_hist_nums = []
                freq_map = {n: 0 for n in range(1, 39)}
            ...
            while len(final_base) < 6:
                if candidate_pool:
                    ...
                    r_n = next(n for n in range(1, 39) if n not in final_base)
            ...
            # 6. 特別號生成邏輯
            if user_nums_bazi_b:
                special_num = user_nums_bazi_b[0] # 使用者有指定第二區
            elif use_bazi and bazi_lucky_pool:
                # 大樂透特別號優先從五行池挑選
                special_num = _top_special_from_df(df_hist if 'df_hist' in locals() else pd.DataFrame(), 8) 
            else:
                special_num = _top_special_from_df(df_hist if 'df_hist' in locals() else pd.DataFrame(), 8)
        ...
        if st.button("🔮 解析夢境並產生推薦", key="top_dream_btn"):
            ...
            if not dream_text:
                ...
                dream_analysis = analyze_dream_text(dream_text, max_num=38, include_resonance=True)
            ...
            if not full_df.empty:
                ...
                for i in range(1, 3):
                    ...
                    while len(current_set) < 6:
                        current_set.add(next(n for n in range(1, 39) if n not in current_set))
                            
                    suggested_list = sorted(list(current_set))
                        
                    # 特別號：取歷史最常開出的特別號
                    special_num = _top_special_from_df(full_df, 8)
