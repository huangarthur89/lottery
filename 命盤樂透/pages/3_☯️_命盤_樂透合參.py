...
def parse_dream_to_numbers(user_input):
    ...
    dream_database = {
        ...
        ("兔", "兔子", "野兔"): [4, 16, 28, 40],
        ("龍", "神龍", "恐龍"): [5, 17, 29, 41],
        ("蛇", "蟒蛇", "毒蛇"): [6, 9, 16, 18, 30, 42],  # 蛇身彎曲逼 06、09，也常逼 16
        ("馬", "白馬", "賽馬"): [7, 19, 31, 43],
        ("羊", "山羊", "綿羊"): [8, 20, 32, 44],
        ("猴", "猴子", "猩猩"): [9, 21, 33, 45],
        ("雞", "公雞", "母雞", "小雞"): [10, 22, 34, 46],
        ("狗", "犬", "小狗", "黑狗"): [9, 11, 23, 35, 47], # 狗叫聲(kau)逼 09
        ...
        ("結婚", "新娘", "喜事", "婚禮"): [26, 36],
        ("懷孕", "孕婦", "生小孩", "大肚子"): [0, 10, 20], # 肚子圓圓逼 0 尾
        ("地震", "搖晃", "地動"): [7, 17, 27],
        ("飛機", "飛行", "出國"): [2, 12, 22],
    ...
    for keywords, nums in dream_database.items():
...
with st.sidebar:
    ...
    time_mode = st.radio("出生時間設定", ["精確時間", "吉時 (不知時辰)"], horizontal=True)
    if "吉時" in time_mode:
        b_time = datetime(1971, 1, 1, 12, 0).time()
        st.info("💡 自動設定為午時 (12:00)。")
    else:
        b_time = st.time_input("出生時間", datetime(1971, 1, 1, 0, 0).time())
