...
def parse_dream_to_numbers(user_input):
    ...
    from dream_dict import analyze_dream_text
    return analyze_dream_text(user_input, max_num=49, include_resonance=False)["numbers"]

# ==========================================
# 旗艦版：夢境靈數共振擴張引擎 (字典 + 密碼 + 固定延伸)
# ==========================================
def expand_dream_resonance(user_input, base_numbers, max_num=49):
    """
    由 dream_dict.analyze_dream_text 統一處理：民間夢字典、顏色/情緒/動作/方位/時間密碼、
    明示數字、鏡像牌、合數、鄰號與同尾數延伸；全程固定規則，不使用亂數。
    """
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from dream_dict import analyze_dream_text
    return analyze_dream_text(user_input, max_num=max_num, include_resonance=True)["numbers"]
...
st.markdown("""
    ...
    /* 隱藏 Streamlit 重新載入時的反灰 (stale) 視覺效果 */
    [data-stale="true"] {
        opacity: 1 !important;
...
def load_game_lottery_stats(game_choice):
    ...
    # 【防呆機制】如果找不到資料庫，改用確定性空資料降級，避免產生不可回溯的亂數。
...
def _fallback_mock_data(game_choice):
    """資料庫異常時的確定性降級方案：只回傳空統計，不製造假資料。"""
    max_n = 49 if "大樂透" in game_choice else 38
    f_data = {n: 0 for n in range(1, max_n + 1)}
    o_data = {n: 100 for n in range(1, max_n + 1)}
    z2_data = {n: 0 for n in range(1, 9)} if "威力彩" in game_choice else None
...
if generate_btn:
    ...
    if dream_input:
        from dream_dict import analyze_dream_text
        dream_analysis = analyze_dream_text(dream_input, max_num=max_num, include_resonance=True)
        dream_nums_for_engine = [n for n in dream_analysis["numbers"] if 1 <= n <= max_num]
        
        if dream_nums_for_engine:
            st.success(f"🔮 潛意識共振捕捉成功！已將號碼注入運算核心：{', '.join(map(str, dream_nums_for_engine))}")
            with st.expander("🔎 夢境密碼來源拆解", expanded=False):
                if dream_analysis.get("matched_descs"):
                    st.write("**字典命中：**")
                    for desc in dream_analysis["matched_descs"][:8]:
                        st.markdown(f"- {desc}")
                ...
                if dream_analysis.get("code_hits"):
                    ...
                    for hit in dream_analysis["code_hits"][:8]:
                        st.caption(f"{hit['label']} → {hit['nums']}｜{hit['reason']}")
        else:
