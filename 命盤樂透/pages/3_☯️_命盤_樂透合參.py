...
with st.sidebar:
    ...
    st.info("💡 **資深彩迷解夢法**：系統已全面支援「網路十大夢牌」（如：蛇=財運夢、水=偏財夢、結婚=喜事夢、金錢=財神夢），並內建民間最流行的**「生肖法」**、**「尾數法」**及強大的**「組合夢法」**。請盡情描述您的夢境元素！")
    ...
    is_lotto_ui = "大樂透" in game_choice
    max_num_ui = 49 if is_lotto_ui else 38

    dream_stats = []
    for keys, data in dream_database.items():
        valid_nums = [n for n in data["nums"] if n <= max_num_ui]
        if not valid_nums: continue
        total_hits = sum(freq_data.get(n, 0) for n in valid_nums)
        avg_hits = total_hits / len(valid_nums)
        dream_stats.append({
            "keyword": keys[0],
            "avg_hits": avg_hits,
    ...
    top_dreams = sorted(dream_stats, key=lambda x: x["avg_hits"], reverse=True)[:3]
    
    with st.expander("📊 近期最強發財夢排行榜 (大數據實測)", expanded=False):
        st.markdown(f"<span style='font-size:14px; color:#aaa;'>基於 **{game_choice.split(' ')[0]}** 最新 100 期開獎數據，幫您找出近期最會開的超旺夢牌！</span>", unsafe_allow_html=True)
        for i, d in enumerate(top_dreams):
            st.markdown(f"**Top {i+1}：{d['keyword']}** <span style='color:#FFD700;'>(平均單碼命中 {d['avg_hits']:.1f} 次)</span>", unsafe_allow_html=True)
            st.caption(f"關聯號碼：{d['nums']} | 總開出次數：{d['total_hits']}")
            
    dream_input = st.text_area("請簡述您近期的夢境或強烈直覺：", placeholder="例如：我夢到好多鈔票被小偷拿走，旁邊還有一條蛇跑過去...")
...
if generate_btn:
    ...
    for n in all_nums:
        ...
        # 🌌 核心植入：夢境靈數強制共振加權
        # ==========================================
        # 判斷是否為大吉本命數 (維度二、三、四的強烈特徵)
        is_bazi_lucky = (tail in self_tails) or (tail in favorable_tails) or (n == life_path) or (tail == life_path) or (sum(int(d) for d in str(n)) == life_path)
        
        if dream_nums_for_engine and n in dream_nums_for_engine:
            score += 50  # 超級權重，強勢擠進前段班
            if is_bazi_lucky:
                resonance_nums.append(n)
    ...
    with col1:
        if resonance_nums:
            res_html = format_balls_html(resonance_nums, 'ball-red', zone2_final)
            st.markdown(f"""
            <div class='strategy-card' style='border: 2px solid #FFD700; background: linear-gradient(145deg, #2a1a00 0%, #1a0f00 100%); animation: pulse-gold 2s infinite;'>
                <div class='strategy-title'>
                    <span style='color: #FFD700; font-size: 1.2em; font-weight: bold;'>🔥 天人合一！雙效共振大吉組</span>
                    <span style='color:#eeddcc; font-size:14px; display:block; margin-top:5px; line-height: 1.4;'>
                        太神了！您的潛意識夢境與本命盤產生了極罕見的強烈共鳴！以下號碼既是您的夢牌，也是您的命盤大吉數，請務必把握！
                    </span>
                </div>
                {res_html}
            </div>
...
# ==========================================
st.markdown("---")
st.markdown("### 💰 財富自由！官方連碰包牌資金試算機")
st.markdown("當您獲得了多組強烈共振的號碼，想知道如果把它們「一網打盡」全部包牌需要花多少錢嗎？本試算機完全符合台灣彩券官方計價公式。")

import math

is_lotto_calc = "大樂透" in game_choice

col_calc1, col_calc2 = st.columns([1, 1])

with col_calc1:
    st.info(f"**目前選擇遊戲：** {game_choice}")
    if is_lotto_calc:
        n_balls = st.slider("第一區想包幾個號碼？ (連碰)", min_value=6, max_value=16, value=7, step=1)
        # 大樂透計算
        combinations = math.comb(n_balls, 6)
        cost = combinations * 50
        
        st.markdown(f"#### 📊 試算結果")
        st.markdown(f"- **包牌方式**：大樂透 {n_balls} 連碰")
        st.markdown(f"- **總注數**：`C({n_balls}, 6)` = **{combinations}** 注")
        st.markdown(f"- **總金額**：{combinations} × 50 = <span style='color:#FFD700; font-size:1.5em; font-weight:bold;'>{cost:,}</span> **元**", unsafe_allow_html=True)
        
    else:
        n_balls = st.slider("第一區想包幾個號碼？ (連碰)", min_value=6, max_value=16, value=6, step=1)
        m_balls = st.slider("第二區想包幾個號碼？", min_value=1, max_value=8, value=8, step=1)
        # 威力彩計算
        combinations_zone1 = math.comb(n_balls, 6)
        total_combinations = combinations_zone1 * m_balls
        cost = total_combinations * 100
        
        st.markdown(f"#### 📊 試算結果")
        if n_balls == 6 and m_balls == 8:
            st.markdown("- **包牌方式**：威力彩 第二區全包 (俗稱威力彩小全餐)")
        elif n_balls > 6 and m_balls == 8:
            st.markdown(f"- **包牌方式**：威力彩 {n_balls} 連碰 + 第二區全包 (豪華套餐)")
        else:
            st.markdown(f"- **包牌方式**：威力彩 第一區 {n_balls} 碼 + 第二區 {m_balls} 碼")
            
        st.markdown(f"- **總注數**：`C({n_balls}, 6) × {m_balls}` = **{total_combinations}** 注")
        st.markdown(f"- **總金額**：{total_combinations} × 100 = <span style='color:#FFD700; font-size:1.5em; font-weight:bold;'>{cost:,}</span> **元**", unsafe_allow_html=True)

with col_calc2:
    st.markdown("""
    <div style="background-color: rgba(255, 215, 0, 0.1); border-left: 4px solid #e6b800; padding: 15px; border-radius: 5px;">
        <h4 style="color: #d49a00; margin-top:0;">💡 行家包牌心法</h4>
        <p style="font-size: 14px; line-height: 1.6; color: inherit;">
            <strong>大樂透：</strong>最常見的是 <b>7連碰</b>（350元）或 <b>8連碰</b>（1400元）。因為只要花少許資金，就能把命中率大幅提高！當命中三個號碼時，連碰的注單會有「多重中獎」的疊加效應，報酬率驚人。<br><br>
            <strong>威力彩：</strong>行家最愛 <b>第二區全包</b>（800元）。因為威力彩只要第一區中 1 碼加上第二區中，就有普獎！第二區全包能確保「只要第一區有中獎，絕對不會跟大獎擦身而過」。
        </p>
    </div>
    """, unsafe_allow_html=True)
