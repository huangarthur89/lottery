...
with st.sidebar:
    ...
    with st.form("astro_form"):
        ...
        time_precision = st.radio(
            "出生時間精度",
            ["✅ 知道精確時間", "🕰️ 只知道傳統時辰", "🔮 不知出生時間 (預設午時估算)"],
            index=1
        )
        ...
        if time_precision.startswith("✅"):
            b_time = st.time_input("公曆出生時間", value=datetime(1971, 1, 1, 12, 0).time(), step=timedelta(minutes=1))
            b_hour_str = "精確時間"
        elif time_precision.startswith("🕰️"):
            b_hour_str = st.selectbox("公曆出生時辰", list(time_map.keys()), index=6)
            b_time = time_map[b_hour_str]
            st.caption("只知道時辰時，系統會使用該時辰中點排盤；若出生接近時辰交界，建議改用精確時間。")
        else:
            b_hour_str = "不知時間"
            b_time = datetime(1971, 1, 1, 12, 0).time()
            st.caption("不知出生時間時，系統以午時 12:00 作為估算基準；時柱、身宮、上升星座僅供參考。")
        is_lucky_time = time_precision.startswith("🔮")
...
# ==========================================
if generate_btn:
    # 不知出生時間時，以午時 12:00 作為估算基準。
    if is_lucky_time:
        # 將 b_time 替換為 12:00
        b_time = datetime(1971, 1, 1, 12, 0).time()
        time_mode = "🔮 不知道時間"
        # 在右側主畫面給予溫馨提示
        st.warning("🔮 您勾選了「不知出生時間」，系統已使用「午時 12:00」作為估算基準。請留意，時柱、命宮、身宮與上升星座可能因時間誤差而偏移。")
    ...
    with st.spinner("🌌 正在為您精算天體運行與命盤參數..."):
        ...
        # 動態判斷顯示的時間字眼
        has_known_time = "不知道" not in time_mode
        is_exact_time = "精確" in time_mode
        time_display = "午時估算 (未輸入)" if not has_known_time else b_time.strftime('%H時%M分')
        true_time_display = "--" if not has_known_time else true_datetime.strftime('%Y年%m月%d日 %H時%M分')
        lunar_hour_display = "估" if not has_known_time else ziwei_chart['lunar_hour']

        moon_disclaimer = "<br><span style='font-size:10px; color:#D32F2F;'>*出生時間非精確時，時柱、身宮、上升星座可能偏移</span>" if not is_exact_time else ""
            ...
            <div class="center-hall">
                ...
                <div class="center-text">
                    <div class="center-grid">
                        ...
                        <div class="center-label">節氣四柱：</div><div class="center-value" style="grid-column: span 3; font-size:16px; color:#B71C1C;">{eight_char.getYear()}年 {eight_char.getMonth()}月 {eight_char.getDay()}日 {eight_char.getTime() if has_known_time else "**午時估算**"}</div>
        ...
        if "不知道" not in time_mode: all_wuxing += eight_char.getTimeWuXing()
        count_elements = {e: all_wuxing.count(e) for e in "金木水火土"}
        
        favorable_elements = [k for k, v in count_elements.items() if v <= 1] # 四柱中出現較少的五行，非正式用神
        if not favorable_elements: favorable_elements = ["五行分布均衡"]
        ...
        <div style="max-width: 1400px; margin: 40px auto; background-color: #FFF3E0; padding: 30px; border-radius: 15px; border-left: 8px solid #FF8F00; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 20px; font-size: 20px; color: #424242; font-family: 'MingLiU', 'PMingLiU', 'Noto Serif TC', serif;">
                <div style="flex: 1; min-width: 250px; background: #fff; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                    ...
                    <div style="font-size: 16px; color: #757575; margin-top: 10px;">(此為日干支方位提示，重要決策仍需以現實條件為主)</div>
                </div>
                <div style="flex: 1; min-width: 250px; background: #fff; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                    <div style="font-weight: bold; color: #1565C0; margin-bottom: 10px;">🎨 今日幸運密碼</div>
                    四柱較少五行：<b style="color: #1976D2;">{'、'.join(favorable_elements)}</b><br>
                    <div style="font-size: 16px; color: #757575; margin-top: 10px;">(此為五行計數提示，非完整喜用神判斷；水為藍黑、木為青綠、火為紅紫、土為黃棕、金為白金)</div>
                </div>
            </div>
        </div>
        '''
        ...
        # 6. 時間擴充：五行能量圖表
        # ==========================================
        st.markdown("<br><h2 style='color:#000; font-family:serif; text-align:center; font-weight:bold; font-size:26px;'>📊 流年五行能量模型 (1990 - 2040)</h2>", unsafe_allow_html=True)
        # 1. 取得命主日元與五行
        day_master = eight_char.getDayGan()
        ...
        # 3. 流年五行權重定義：天干 40 分，地支依藏干主/中/餘氣加權
        STEM_ELEMENT = {
        ...
        def get_real_energy(target_year, target_element):
            ...
            stem, branch = year_ganzhi[0], year_ganzhi[1]
            score = 20
            if STEM_ELEMENT.get(stem) == target_element:
                score += 40
            score += sum(weight for element, weight in BRANCH_HIDDEN_ELEMENTS.get(branch, []) if element == target_element)
            return score
        ...
        def build_modern_report():
            ...
            asc_text = astro_data['asc'] if is_exact_time else ('時辰中點估算' if has_known_time else '需精確時間')
