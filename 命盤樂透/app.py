...
# 1. 頁面基礎設定與全域路由 CSS
# ==========================================
st.set_page_config(page_title="天機合參 · 命盤樂透整合系統", page_icon="🏠", layout="wide", initial_sidebar_state="collapsed")
...
with col1:
    if st.button("🔮 命盤分析\n\n深究紫微八字\n掌握靈魂軌跡", use_container_width=True):
        st.switch_page("pages/1_🔮_命盤分析.py")

with col2:
    if st.button("📊 樂透分析\n\n大數據監測\n冷熱號碼統計", use_container_width=True):
        st.switch_page("pages/2_🎰_樂透分析.py")

with col3:
    if st.button("🌌 命盤 × 樂透合參\n\n五行靈數共振\n專屬高勝率策略", use_container_width=True):
        st.switch_page("pages/3_☯️_命盤_樂透合參.py")
