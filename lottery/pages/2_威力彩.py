...
# --- 核心爬蟲與清洗邏輯 ---
def fetch_super_lotto_data(year, month):
...
if st.sidebar.button("🚀 開始抓取並寫入資料庫"):
    if start_date > end_date:
        ...
        for i, (y, m) in enumerate(query_months):
            ...
            month_data = fetch_super_lotto_data(y, m)
...
if not full_df.empty:
        ...
        if st.button("🚀 開始回測"):
            if full_df.empty:
                ...
                try:
                    ...
                    if len(my_nums) != 6 or not all(1 <= x <= 38 for x in my_nums):
                        st.warning("請確保輸入的是『剛好 6 個』且『介於 1~49 之間』的不重複號碼喔！")
                    else:
            ...
            if full_df.empty:
                ...
                try:
                    ...
                    if len(check_nums) != 6 or not all(1 <= x <= 38 for x in check_nums):
                        ...
                        elif 80 <= sum_val < 95 or 139 < sum_val <= 150:
                            score -= 10
    ...
    st.download_button("📥 下載完整 Excel/CSV 檔案", csv, "super_lotto_analysis.csv", "text/csv")
