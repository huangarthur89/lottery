...
ui_code_template = """        st.markdown("基於歷史大數據庫，為您自動產生的不同策略預測號碼：")
        ...
        # 6. 近期動能
        momentum_6 = strategy_short_term_momentum(full_df, pool_size={POOL_SIZE}, recent_draws=50)
        
        # 7. 斷區殺牌
        zone_6 = strategy_zone_elimination(full_df, pool_size={POOL_SIZE})
        
        # 8. 綜合 AI
        composite_6 = strategy_composite_ai(full_df, pool_size={POOL_SIZE})
        
        # 9. 絕對冷門
        cold_6 = strategy_absolute_cold(full_df, pool_size={POOL_SIZE})
        
        # 產生特別號 (從前 5 大熱門中隨機挑選)
        special_pool = [n for n, c in Counter(full_df['特別號']).most_common(5)]
        if not special_pool:
            special_pool = list(range(1, {SPECIAL_MAX} + 1)) # Fallback
        ...
        pred_data = [
            ...
            ["📈 近期動能 (近50期強勢號)", *momentum_6, random.choice(special_pool)],
            ["🕳️ 斷區殺牌 (強制空區法)", *zone_6, random.choice(special_pool)],
            ["🧠 綜合 AI (多因子權重)", *composite_6, random.choice(special_pool)],
            ["❄️ 絕對冷門 (反向操作)", *cold_6, random.choice(special_pool)]
        ]
...
def process_file(file_path, pool_size, min_sum, max_sum, special_max):
    ...
    if start_idx != -1 and end_idx != -1:
        ...
        new_block = ui_code_template.replace('{POOL_SIZE}', str(pool_size)).replace('{MIN_SUM}', str(min_sum)).replace('{MAX_SUM}', str(max_sum)).replace('{SPECIAL_MAX}', str(special_max))
...
process_file('pages/1_大樂透.py', 49, 130, 170, 49)
process_file('pages/2_威力彩.py', 38, 95, 139, 8)
