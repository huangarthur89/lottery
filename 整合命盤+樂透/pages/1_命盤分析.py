...
with st.sidebar:
    ...
    latitude = st.number_input("緯度 (北緯)", value=25.03, step=0.01, format="%.2f")
    ...
    <div class="center-hall">
        ...
        <div class="center-text">
            <div class="center-grid">
                ...
                <div class="center-label">節氣四柱：</div><div class="center-value" style="grid-column: span 3; font-size:16px; color:#B71C1C;">{eight_char.getYear()}年 {eight_char.getMonth()}月 {eight_char.getDay()}日 {eight_char.getTime()}時</div>
...
<div class="astro-container">
    ...
    <div class="astro-card"><div class="astro-title">🌙 月亮 (潛意識)</div><div class="astro-value">{astro_data['moon']}</div><div class="astro-element" style="background-color: #0D47A1;">情緒與安全感</div><div style="font-size:12px; color:#666; margin-top:5px;">黃經 {astro_data['moon_deg']:.2f}°</div></div>
    <div class="astro-card"><div class="astro-title">✨ 上升 (社會人格)</div><div class="astro-value">{astro_data['asc']}</div><div class="astro-element" style="background-color: #FBC02D; color:#000;">面具與舵手</div><div style="font-size:12px; color:#666; margin-top:5px;">黃經 {astro_data['asc_deg']:.2f}°</div></div>
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=years, y=energy_water, mode='lines', name='金水能量 (食傷生財)', line=dict(color='#0D47A1', width=4, shape='spline'), hoverinfo="text", hovertext=hover_water, hovertemplate="%{hovertext}<extra></extra>"))
fig.add_trace(go.Scatter(x=years, y=energy_fire, mode='lines+markers', name='火土能量 (印星護身)', line=dict(color='#B71C1C', width=3, dash='dot', shape='spline'), hoverinfo="text", hovertext=hover_fire, hovertemplate="%{hovertext}<extra></extra>"))
...
fig.update_layout(
    ...
    hovermode="x unified",
    xaxis=dict(title=dict(text='<b>西元年份</b>', font=dict(size=16, color="#000")), tickfont=dict(size=14, color="#000"), showgrid=True, gridcolor='#DDD', showspikes=True, spikemode="across", spikethickness=1, spikedash="solid", spikecolor="#666666"),
    yaxis=dict(title=dict(text='<b>能量指數</b>', font=dict(size=16, color="#000")), tickfont=dict(size=14, color="#000"), showgrid=True, gridcolor='#DDD'),
...
<div style="border: 3px solid #000000; background-color: #FAFAFA; padding: 25px; border-radius: 4px; font-family: 'PMingLiU', serif; color:#000;">
    <h3 style="color: #000; margin-top:0; border-bottom:2px solid #000; padding-bottom:12px; font-weight:bold; font-size:22px;">🔍 大師級·萬象合參深度診斷報告</h3>
    <div style="font-size: 17px; line-height: 2.0; color: #111; font-weight: 500;">
        <p><b>【盲派命理與神煞奧秘】</b> 透過五虎遁精算，受孕胎元為「{tai_yuan}」，靈魂命宮落於「{ming_gong}」。系統判定命局帶有 <span style="color:#C62828; font-weight:bold;">{'、'.join(shen_sha_list) if shen_sha_list else '無顯著神煞'}</span>。驛馬星主導一生頻繁變動；文昌與桃花結合暗示具備卓越的藝術才華與群眾魅力。</p>
        <p><b>【玄空風水佈局】</b> 檢視 {current_year} 年九宮飛星佈局：大凶災病之「五黃廉貞星」飛臨南方，家中或辦公室南方切忌動土；代表財富的「八白正財星」飛臨東方，建議於東方佈置常坐臥於此，強力催旺事業正財。</p>
        <p><b>【靈數與東西星象】</b> 東方日主為「{day_master}」；西方精算太陽 <span style="color:#1565C0; font-weight:bold;">{astro_data['sun']}</span>、月亮 <span style="color:#1565C0; font-weight:bold;">{astro_data['moon']}</span>、上升 <span style="color:#C62828; font-weight:bold;">{astro_data['asc']}</span>。加上生命靈數「{life_path_num}」之「<span style="color:#4A148C; font-weight:bold;">{lp_title}</span>」天賦。東方看運勢氣場，西方論人格心理，三者合一，精準勾勒出全方位軌跡！</p>
    </div>
</div>
''', unsafe_allow_html=True)
